...
public class ViewQuoteFactoryDigi {
    ...
    public void ViewPageDigi(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        if (testData.get("IsELCDigi").equalsIgnoreCase("YES")) {
            viewquote.SelectENHANCELIFECOVERAGEWeb();
        }
        if (testData.get("IsACIDigi").equalsIgnoreCase("YES")) {
            viewquote.EnterACIAmountWeb(testData.get("ACIumAssuredamountDigi"));
        }
        ...
        if (prop.getProperties("Isjointlife").equalsIgnoreCase("Yes")) {
            ...
            jointlife.enterPANcardjointlife(testData.get("DGJointlifePAN"));
            jointlife.clickOnVerifybuttonjointlifeweb();
            TestUtil.scrollTillTOPOfPage(getDriver());
            jointlife.fillDOBdetailsJointlifeWeb(testData.get("DGJointlifeday"), testData.get("DGJointlifemonth"), testData.get("DGJointlifeyear"));
            jointlife.ClickOnGenderJointlife(testData.get("DGJointlifegender"));
