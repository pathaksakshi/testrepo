...
public class ViewQuoteFactoryAkshaya {
    Dashboard dashboard;
    WaitUtils wait;
    LoginPage loginPage;
    ...
    public void ViewPageAkshaya(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        if (testData.get("IsPremiumAmountAkshaya").equalsIgnoreCase("YES")) {
            TestUtil.scrollTillTOPOfPage(getDriver());
            wait.WaitTime(5);
            viewquote.EnterPremiumAmountWeb(testData.get("premiumamountAkshaya"));
        }
        if (testData.get("IsBenefitOptionAkshaya").equalsIgnoreCase("YES")) {
            viewquote.SelectbeneoptWeb(testData.get("benefitotionAkshaya"));
        }
        if (testData.get("IsPPTOptionAkshaya").equalsIgnoreCase("YES")) {
            viewquote.SelectPPTWeb(testData.get("pptAkshaya"));
            //viewquote.PopupPPTWeb();
        }
        if (testData.get("IspolicytermAkshaya").equalsIgnoreCase("YES")) {
            viewquote.SelectPoltermAAPWeb(testData.get("policytermAkshaya"));
        }
        if (testData.get("IsBenefitfrequencyAkshaya").equalsIgnoreCase("YES")) {
            viewquote.SelectbenefreqWeb(testData.get("benefitfreqAkshaya"));
        }
        if (testData.get("IsPaymodeAkshaya").equalsIgnoreCase("YES")) {
            viewquote.SelectPaymodeWeb(testData.get("paymodeAkshaya"));
        }

        // Add riders //
        if (testData.get("IsAddRiderAkshaya").equalsIgnoreCase("YES")) {
            viewquote.addriderbutton();

            if (testData.get("IswavierofpremiumAkshaya").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();
            }

                if (testData.get("IsADBRiderAkshaya").equalsIgnoreCase("Accidental Death Benefit Plus")) {
                    viewquote.addADB(testData.get("ADMamountAkshaya"));
                }
                if (testData.get("IsCIRiderAkshaya").equalsIgnoreCase("Critical Illness")) {
                    viewquote.addCI(testData.get("CIamountAkshaya"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
                if (testData.get("IsSCRiderAkshaya").equalsIgnoreCase("Surgical Care")) {
                    viewquote.addSC(testData.get("SCamountAkshaya"));
                }
                if (testData.get("IsHSRiderAkshaya").equalsIgnoreCase("Hospital Care")) {
                    viewquote.addHC(testData.get("HCamountAkshaya"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
