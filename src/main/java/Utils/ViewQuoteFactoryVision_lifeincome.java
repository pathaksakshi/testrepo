...
public class ViewQuoteFactoryVision_lifeincome {
    Dashboard dashboard;
    WaitUtils wait;
    LoginPage loginPage;
    ...
    public void ViewPageVision_lifeincome(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        // View Quote page //
        if ((testData.get("IsSumAssuredvisionlife")).equalsIgnoreCase("YES")) {
            viewquote.EnterSumAssuredWeb(testData.get("SumAssuredvisionlife"));
        }
        if ((testData.get("IsPayingTermvisionlife")).equalsIgnoreCase("YES")) {
            viewquote.EnterPolpayingtermmWeb(testData.get("PayingTermvisionlife"));
        }
        if ((testData.get("IsPaymodevisionlife")).equalsIgnoreCase("YES")) {
            viewquote.SelectPaymodeWeb(testData.get("paymodevisionlife"));
        }

        // Add riders //
        if (testData.get("IsAddRidervisionlife").equalsIgnoreCase("YES")) {
            viewquote.addriderbutton();

            if (testData.get("Iswavierofpremiumvisionlife").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();
            }

                if (testData.get("IsADDRidervisionlife").equalsIgnoreCase("Accidental Death and Disability")) {
                    viewquote.addADD(testData.get("ADDMamountvisionlife"));
                }
                if (testData.get("IsCIRidervisionlife").equalsIgnoreCase("Critical Illness")) {
                    viewquote.addCI(testData.get("CIamountvisionlife"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
                if (testData.get("IsSCRidervisionlife").equalsIgnoreCase("Surgical Care")) {
                    viewquote.addSC(testData.get("SCamountvisionlife"));
                }
                if (testData.get("IsHSRidervisionlife").equalsIgnoreCase("Hospital Care")) {
                    viewquote.addHC(testData.get("HCamountvisionlife"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
                if ((testData.get("IsADBRidervisionlife")).equalsIgnoreCase("Accidental Death Benefit Plus")) {
                    viewquote.addADB(testData.get("ADBamountvisionlife"));
                }
