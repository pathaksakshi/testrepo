...
public class ViewQuoteFactoryASP {
    Dashboard dashboard;
    WaitUtils wait;
    LoginPage loginPage;
    ...
    public void ViewPageASP(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        // View Quote page //
        if (testData.get("IsPremiumAmountASP").equalsIgnoreCase("YES")) {
            viewquote.EnterPremiumAmountWeb(testData.get("premiumamountASP"));
        }
        if (testData.get("IsPPTASP").equalsIgnoreCase("YES")) {
            viewquote.SelectPPTWeb(testData.get("pptASP"));
        }
        if (testData.get("pptASP").equalsIgnoreCase("Single Pay")) {
            viewquote.SelectPlanoptWeb(testData.get("PlanOptionASP"));
        }
        if (testData.get("IspolicytermASP").equalsIgnoreCase("YES")) {
            viewquote.EnterPolicytermASPWeb(testData.get("PolicyTermASP"));
        }
        if (testData.get("IsPaymodeASP").equalsIgnoreCase("YES")) {
            viewquote.SelectPaymodeWeb(testData.get("paymodeASP"));

        }
        // JOINT LIFE DETAILS //
        TestUtil.scrollTillEndOfPage(getDriver());

        if (prop.getProperties("Isjointlife").equalsIgnoreCase("Yes")) {
            jointlife.clickOnAddbuttonjointlifeweb();
            jointlife.enterPANcardjointlife(testData.get("ASPJointlifePAN"));
            jointlife.clickOnVerifybuttonjointlifeweb();
            TestUtil.scrollTillTOPOfPage(getDriver());
            jointlife.fillDOBdetailsJointlifeWeb(testData.get("ASPJointlifeday"), testData.get("ASPJointlifemonth"), testData.get("ASPJointlifeyear"));
            jointlife.ClickOnGenderJointlife(testData.get("ASPJointlifegender"));
            jointlife.clickOnsavebuttonjointlifeweb();
        }
