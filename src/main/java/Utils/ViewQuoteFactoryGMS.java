...
public class ViewQuoteFactoryGMS {
	 ...
	 Dashboard dashboard;
	    ...
	    public void ViewPageGMS(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
			...
			jointlife = new JointlifePage(getDriver());
			...
			if (prop.getProperties("Isjointlife").equalsIgnoreCase("Yes")) {
				...
				jointlife.enterPANcardjointlife(testData.get("GMSJointlifePAN"));
				jointlife.clickOnVerifybuttonjointlifeweb();
				TestUtil.scrollTillEndOfPage(getDriver());
				jointlife.fillDOBdetailsJointlifeWeb(testData.get("GMSJointlifeday"), testData.get("GMSJointlifemonth"), testData.get("GMSJointlifeyear"));
				jointlife.ClickOnGenderJointlife(testData.get("GMSJointlifegender"));
