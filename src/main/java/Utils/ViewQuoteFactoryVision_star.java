...
public class ViewQuoteFactoryVision_star {
    Dashboard dashboard;
    WaitUtils wait;
    LoginPage loginPage;
    ...
    public void ViewPageVision_star(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        if (testData.get("Issumaasuredstar").equalsIgnoreCase("YES")) {
            viewquote.enterSumAssuredWeb(testData.get("SumAssuredAmountstar"));
            wait.WaitTime2();
        }

        if (testData.get("IsPlanOptionstar").equalsIgnoreCase("YES")) {
            viewquote.SelectPlanOptionweb(testData.get("PlanOptionstar"));
            wait.WaitTime2();
        }

        if (testData.get("IsPPTOptionstar").equalsIgnoreCase("YES")) {
            viewquote.SelectPPTWeb(testData.get("pptOptionstar"));
            wait.WaitTime2();
        }

        if (testData.get("IsPaymodestar").equalsIgnoreCase("YES")) { // For Single pay only Annual is there.
            viewquote.SelectPaymodeWeb(testData.get("paymodestar"));
        }

        // Add riders //
        if (testData.get("IsAddRiderstar").equalsIgnoreCase("YES")) {
            viewquote.addriderbutton();

            if (testData.get("Iswavierofpremiumstar").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();

            }

                if (testData.get("IsADDRiderstar").equalsIgnoreCase("Accidental Death and Disability")) {
                    viewquote.addADD(testData.get("ADDMamountstar"));
                }
                if (testData.get("IsCIRiderstar").equalsIgnoreCase("Critical Illness")) {
                    viewquote.addCI(testData.get("CIamountstar"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
                if (testData.get("IsSCRiderstar").equalsIgnoreCase("Surgical Care")) {
                    viewquote.addSC(testData.get("SCamountstar"));
                }
                if (testData.get("IsHSRiderstar").equalsIgnoreCase("Hospital Care")) {
                    viewquote.addHC(testData.get("HCamountstar"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
                if ((testData.get("IsADBRiderstar")).equalsIgnoreCase("Accidental Death Benefit Plus")) {
                    viewquote.addADB(testData.get("ADBamountstar"));
                }
