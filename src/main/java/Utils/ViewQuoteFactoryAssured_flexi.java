...
public class ViewQuoteFactoryAssured_flexi {
    Dashboard dashboard;
    WaitUtils wait;
    LoginPage loginPage;
    ...
    public void ViewPageAssured_flexi(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        if (testData.get("IsPremiumAmountflexi").equalsIgnoreCase("YES")) {
            viewquote.EnterPremiumAmountWeb(testData.get("premiumamountflexi"));
        }
        if (testData.get("IsPPTOptionflexi").equalsIgnoreCase("YES")) {
            viewquote.SelectPPTWeb(testData.get("pptflexi"));
        }
        if (testData.get("IsPaymodeflexi").equalsIgnoreCase("YES")) {
            viewquote.SelectPaymodeWeb(testData.get("paymodeflexi"));
        }

        // Add riders //
        if (testData.get("IsAddRiderflexi").equalsIgnoreCase("YES")) {
            viewquote.addriderbutton();

            if (testData.get("Iswavierofpremiumflexi").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();
            }

                if (testData.get("IsADBRiderflexi").equalsIgnoreCase("Accidental Death Benefit Plus")) {
                    viewquote.addADB(testData.get("ADMamountflexi"));
                }
                if (testData.get("IsCIRiderflexi").equalsIgnoreCase("Critical Illness")) {
                    viewquote.addCI(testData.get("CIamountflexi"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
                if (testData.get("IsSCRiderflexi").equalsIgnoreCase("Surgical Care")) {
                    viewquote.addSC(testData.get("SCamountflexi"));
                }
                if (testData.get("IsHSRiderflexi").equalsIgnoreCase("Hospital Care")) {
                    viewquote.addHC(testData.get("HCamountflexi"));
                    TestUtil.scrollTillEndOfPage(getDriver());
                }
