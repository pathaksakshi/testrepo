...
public class TestUtil extends TestBase {
	...
	public void switchToFrame() {
		BasePage.getDriver().switchTo().frame("mainpanel");
	}
	...
	public static void runTimeInfo(String messageType, String message) throws InterruptedException {
		js = (JavascriptExecutor) BasePage.getDriver();
...
//		js.executeScript("$.growl.warning({ title: 'Warning!', message: 'your warning message goes here' });");
	...
	public static void scrollTillTOPOfPage(WebDriver driver) {
		...
		((JavascriptExecutor) BasePage.getDriver()).executeScript("window.scrollTo(document.body.scrollHeight,0)");
	...
	public static void scrollTillEndOfPage(WebDriver driver) {
		JavascriptExecutor js = (JavascriptExecutor) BasePage.getDriver();
		((JavascriptExecutor) BasePage.getDriver()).executeScript("window.scrollTo(0,document.body.scrollHeight)");

	}

	public static void verifyRedbgColor(WebElement element, WebDriver driver) throws InterruptedException {
		((JavascriptExecutor) BasePage.getDriver()).executeScript("arguments[0].style.border='3px solid yellow'", element);
		Thread.sleep(2000);
		((JavascriptExecutor) BasePage.getDriver()).executeScript("arguments[0].style.border='3px solid crimson'", element);
	}
	public static void verifybgColor(WebElement element, WebDriver driver) throws InterruptedException {
		((JavascriptExecutor) BasePage.getDriver()).executeScript("arguments[0].style.border='3px solid yellow'", element);
		Thread.sleep(2000);
		((JavascriptExecutor) BasePage.getDriver()).executeScript("arguments[0].style.border='3px solid white'", element);
	}
	public static void scrollToElement(WebDriver driver, WebElement element) {
		Actions a = new Actions(BasePage.getDriver());
	...
	public ArrayList<String> openNewTab(WebDriver driver, String url) {
		...
		ArrayList<String> tabs = new ArrayList<String>(BasePage.getDriver().getWindowHandles());
		BasePage.getDriver().switchTo().window(tabs.get(1));
		BasePage.getDriver().get(url);
		return tabs;
	}

	public static void chooseActionButton(WebElement element) {
	...
	//	String urlname = "file:///Users/shubhammishra/Downloads/"+splitStr[0];
		...
		ListingScreenFrontEnd listingScreenFrontEnd = new ListingScreenFrontEnd(BasePage.getDriver());
		readFromPDF(getappNum,premium,listingScreenFrontEnd.getCustomerName(),1, urlname,fileName,base64);
	}
*/
	/*public void readFromPDF(String applicationNum, String Amount, String Name, int pageNum, String urlname, String fileName, String base64) throws IOException, InterruptedException {
	...
	//	Assert.assertTrue(pdfText.contains(Amount));
	...
	public static void switchTowindow(int WindowNumber) {
		...
		try {
			...
			getDriver().switchTo().window(win[WindowNumber].toString());
